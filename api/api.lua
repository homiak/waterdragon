...
-- Упрощенная система поклона для Шотландского Дракона
local S = waterdragon.S or function(s) return s end
...
minetest.register_chatcommand("get_wtd_by_id", {
    params = "<wtd_id>",
    description = S("Teleports your Water Dragon to you by its ID"),
    func = function(name, param)
        ...
        local wtd_id = param
        if not wtd_id or wtd_id == "" then
            return false, S("Please provide a Water Dragon ID")
        end
        ...
        if dragon_data.owner ~= name then
            return false, S("This Water Dragon doesn't belong to you")
        end

        local player_pos = player:get_pos()
        local teleport_pos = {x = player_pos.x, y = player_pos.y + 1, z = player_pos.z}

        -- Check if the dragon is currently loaded
        local dragon = nil
        for _, obj in pairs(minetest.luaentities) do
            if obj.wtd_id == wtd_id and (obj.name == "waterdragon:pure_water_dragon" or obj.name == "waterdragon:rare_water_dragon") then
                dragon = obj
                break
            end
        end
        ...
        if dragon and dragon.object then
            -- If the dragon is loaded, teleport it directly
            dragon.object:set_pos(teleport_pos)
            ...
            dragon.object:set_yaw(yaw)

            -- Set the "stand" animation if the animate method exists
            if dragon.animate then
                dragon:animate("stand")
            end
        else
            -- If the dragon is not loaded, update its position in the global data
            waterdragon.waterdragons[wtd_id].last_pos = teleport_pos
            
            -- Force the area to load
            minetest.get_voxel_manip():read_from_map(teleport_pos, teleport_pos)
            
            -- Spawn the dragon at the new position
            local dragon_type = dragon_data.name or "waterdragon:rare_water_dragon"
            local new_dragon = minetest.add_entity(teleport_pos, dragon_type)
            
            if new_dragon then
                local ent = new_dragon:get_luaentity()
                ent.wtd_id = wtd_id
                -- Restore other necessary data from dragon_data
                ent.owner = dragon_data.owner
                ent.nametag = dragon_data.name
                -- Add any other necessary data restoration here
                
                -- Set the dragon's yaw to match the player's direction
                local player_look_dir = player:get_look_dir()
                local yaw = minetest.dir_to_yaw(player_look_dir)
                new_dragon:set_yaw(yaw)
