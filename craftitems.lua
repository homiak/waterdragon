...
for color in pairs(waterdragon.colors_pure_water) do
	...
	mobforge.register_mob("waterdragon:egg_pure_water_dragon", {
		-- Stats
		max_health = 30,
		...
		step_func = function(self, dtime)
			...
			local name = mobforge.get_node_def(pos).name
...
for color in pairs(waterdragon.colors_rare_water) do
	...
	mobforge.register_mob("waterdragon:egg_rare_water_dragon", {
		-- Stats
		max_health = 30,
		armor_groups = { immortal = 1 },
		despawn_after = false,
...
local function dragon_horn_place(itemstack, player, pointed_thing)
	...
	local node_def = mobforge.get_node_def(under)
