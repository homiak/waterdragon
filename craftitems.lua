...
-- Определение яйца шотландского дракона
minetest.register_node("waterdragon:egg_scottish", {
    description = S("Scottish Dragon Egg"),
    drawtype = "mesh",
    mesh = "waterdragon_egg.b3d",
    tiles = {"waterdragon_scottish_dragon_egg_mesh.png"},
    inventory_image = "waterdragon_scottish_dragon_egg.png",
    wield_image = "waterdragon_scottish_dragon_egg.png",
    paramtype = "light",
    sunlight_propagates = true,
    walkable = true,
    selection_box = {
    ...
    groups = {snappy = 2, falling_node = 1, oddly_breakable_by_hand = 3},
    sounds = default.node_sound_stone_defaults(),
    on_place = function(itemstack, placer, pointed_thing)
        local pos = pointed_thing.above
        local below_pos = {x = pos.x, y = pos.y - 1, z = pos.z}
        local below_node = minetest.get_node(below_pos)
        if below_node.name == "waterdragon:bone_pile_wet" then
            minetest.set_node(pos, {name = "waterdragon:egg_scottish"})
            minetest.get_node_timer(pos):start(1020) -- 17 minutes
            itemstack:take_item()
            return itemstack
        end
    ...
    on_timer = function(pos, elapsed)
        ...
        if water_count == 9 then
            minetest.set_node(pos, {name = "air"})
            local dragon = minetest.add_entity(pos, "waterdragon:scottish_dragon")
            if dragon then
                local ent = dragon:get_luaentity()
                ent.growth_scale = 0.1
                ent.textures = ent.child_textures
                ent:set_scale(0.1)
            end
            return false
        end
