...
waterdragon.registered_movement_methods = {}

function waterdragon.register_movement_method(name, func)
	waterdragon.registered_movement_methods[name] = func
end

waterdragon.registered_utilities = {}

function waterdragon.register_utility(name, func)
	waterdragon.registered_utilities[name] = func
end
...
function waterdragon.is_valid(mob)
...
function waterdragon.is_alive(mob)
	if not waterdragon.is_valid(mob) then
		return false
	end
...
function waterdragon.get_node_height_from_def(name)
...
function waterdragon.get_node_def(node) -- Node can be name or pos
	if type(node) == "table" then
		node = get_node(node).name
	end
	local def = minetest.registered_nodes[node] or default_node_def
	if def.walkable
	and waterdragon.get_node_height_from_def(node) < 0.26 then
...
function waterdragon.get_ground_level(pos, range)
	range = range or 2
...
function waterdragon.is_pos_moveable(pos, width, height)
	local edge1 = {
		x = pos.x - (width + 0.2),
		y = pos.y,
		z = pos.z - (width + 0.2),
		...
		for x = edge1.x, edge2.x do
			...
			for pointed_thing in ray do
				if pointed_thing.type == "node" then
					...
					if waterdragon.get_node_def(name).walkable then
						return false
					end
				end
			end
...
function waterdragon.is_blocked(pos, width, height)
	if width <= 0.5 then
...
function waterdragon.fast_ray_sight(pos1, pos2, water)
	local ray = minetest.raycast(pos1, pos2, false, water or false)
	local pointed_thing = ray:next()
	while pointed_thing do
		if pointed_thing.type == "node"
		and waterdragon.get_node_def(pointed_thing.under).walkable then
...
local fast_ray_sight = waterdragon.fast_ray_sight

function waterdragon.sensor_floor(self, range, water)
...
function waterdragon.sensor_ceil(self, range, water)
...
function waterdragon.get_nearby_player(self, range)
	local pos = self.object:get_pos()
	...
	for _, object in ipairs(objects) do
		if object:is_player()
		and waterdragon.is_alive(object) then
			return object
		end
	end
end

function waterdragon.get_nearby_players(self, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local stored = self._nearby_obj or {}
	local objects = (#stored > 0 and stored) or self:store_nearby_objects(range)
...
function waterdragon.get_nearby_object(self, name, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local stored = self._nearby_obj or {}
	local objects = (#stored > 0 and stored) or self:store_nearby_objects(range)
	for _, object in ipairs(objects) do
		local ent = waterdragon.is_alive(object) and object:get_luaentity()
		if ent
...
function waterdragon.get_nearby_objects(self, name, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local stored = self._nearby_obj or {}
	local objects = (#stored > 0 and stored) or self:store_nearby_objects(range)
...
waterdragon.get_nearby_entity = waterdragon.get_nearby_object
waterdragon.get_nearby_entities = waterdragon.get_nearby_objects

--------------------
-- Global Mob API --
--------------------

function waterdragon.default_water_physics(self)
...
function waterdragon.default_vitals(self)
	local pos = self.stand_pos
...
function waterdragon.basic_punch_func(self, puncher, tflp, tool_caps, dir)
...
function waterdragon.mob_follow(self, player)
...
function waterdragon.mob_stay(self)
    self.object:set_velocity({x=0, y=0, z=0})
end

function waterdragon.mob_wander(self, dtime)
