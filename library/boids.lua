...
function waterdragon.get_boid_cached(self)
...
function waterdragon.get_boid_dir(self)
	local pos = self.object:get_pos()
	if not pos then return end
	local boids = waterdragon.get_boid_cached(self)
...
function waterdragon.get_boid_members(pos, radius, name)
	local objects = minetest.get_objects_inside_radius(pos, radius)
	if #objects < 2 then return {} end
	local members = {}
...
function waterdragon.get_boid_angle(self, _boids, range)
	local pos = self.object:get_pos()
	local boids = _boids or waterdragon.get_boid_members(pos, range or 4, self.name)
