...
--[[local function debugpart(pos, time, tex)
	minetest.add_particle({
		pos = pos,
		texture = tex or "waterdragon_particle_blue.png",
...
local get_node_def = waterdragon.get_node_def
--local get_node_height = waterdragon.get_node_height_from_def

function waterdragon.get_collision(self, dir, range)
...
waterdragon.get_collision_ranged = waterdragon.get_collision

local get_collision = waterdragon.get_collision
...
function waterdragon.get_avoidance_lift(self, pos2, range)
...
function waterdragon.get_avoidance_lift_aquatic(self, pos2, range)
	range = ceil(max(range or 1, 0.5))
	local height_half = (self.height or 1) * 0.5
	local center_y = pos2.y + height_half
	local check_pos = { x = pos2.x, y = center_y, z = pos2.z }
...
function waterdragon.get_context_default(self, goal, steer_dir, interest, danger, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local width, height = self.width or 0.5, self.height or 1
...
function waterdragon.get_context_large(self, goal, steer_dir, interest, danger, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local width, height = self.width or 0.5, self.height or 1
	local y_offset = math.min(self.stepheight or height)
	pos.y = pos.y + y_offset
	local collision = waterdragon.get_collision(self, steer_dir, range)
...
function waterdragon.get_context_small(self, goal, steer_dir, interest, danger, range)
	local pos = self.object:get_pos()
	if not pos then return end
...
function waterdragon.get_context_small_aquatic(self, goal, steer_dir, interest, danger, range)
	local pos = self.object:get_pos()
	if not pos then return end
	pos = vector.round(pos)
	local width = self.width or 0.5
...
function waterdragon.calc_steering(self, goal, get_context, range)
	if not goal then return end
	get_context = get_context or waterdragon.get_context_default
	local pos, yaw = self.object:get_pos(), self.object:get_yaw()
...
function waterdragon.get_context_steering(self, goal, range, water)
	...
	return waterdragon.calc_steering(self, goal, context, range)
end
...
function waterdragon.action_move(self, pos2, timeout, method, speed_factor, anim)
...
waterdragon.action_walk = waterdragon.action_move -- Support for outdated versions

-- Idle

function waterdragon.action_idle(self, time, anim)
	local timer = time
	local function func(_self)
...
function waterdragon.action_fallover(self)
	local zrot = 0
	local init = false
	local dir = 1
...
waterdragon.register_movement_method("waterdragon:pathfind_theta", function(self)
	local path = {}
	local steer_to
	local steer_int = 0
	...
	local function func(_self, goal, speed_factor)
		...
		steer_to = path_dir or (steer_int <= 0 and waterdragon.calc_steering(_self, goal)) or steer_to
...
waterdragon.register_movement_method("waterdragon:pathfind", function(self)
	local path = {}
	local steer_to
	local steer_int = 0
	local arrival_threshold = clamp(self.width, 0.5, 1)
...
waterdragon.register_movement_method("waterdragon:steer_small", function(self)
	local steer_to
	local steer_int = 0
	self:set_gravity(-9.8)
	local function func(_self, goal, speed_factor)
		...
		local speed = abs(_self.speed or 2) * speed_factor or 0.5
		steer_int = (steer_int > 0 and steer_int - _self.dtime) or 1 / math.max(speed, 1)
		steer_to = (steer_int <= 0 and waterdragon.calc_steering(_self, goal)) or steer_to
		-- Apply Movement
		_self:turn_to(dir2yaw(steer_to or vec_dir(pos, goal)), turn_rate)
		_self:set_forward_velocity(speed)
	end
...
waterdragon.register_movement_method("waterdragon:steer_large", function(self)
	local steer_to
	local steer_int = 0
	self:set_gravity(-9.8)
	local function func(_self, goal, speed_factor)
		...
		local speed = abs(_self.speed or 2) * speed_factor or 0.5
		steer_int = (steer_int > 0 and steer_int - _self.dtime) or 1 / math.max(speed, 1)
		steer_to = (steer_int <= 0 and waterdragon.calc_steering(_self, goal, waterdragon.get_context_large)) or steer_to
		-- Apply Movement
		_self:turn_to(dir2yaw(steer_to or vec_dir(pos, goal)), turn_rate)
		_self:set_forward_velocity(speed)
	end
...
waterdragon.register_movement_method("waterdragon:walk_simple", function(self)
	self:set_gravity(-9.8)
	local function func(_self, goal, speed_factor)
		local pos = _self.object:get_pos()
		if not pos or not goal then return end
...
waterdragon.register_movement_method("waterdragon:context_based_steering", function(self)
	local steer_to
	local steer_int = 0
	self:set_gravity(-9.8)
	local function func(_self, goal, speed_factor)
