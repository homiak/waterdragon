...
function mob:indicate_damage()
	...
	minetest.after(0.2, function()
		if waterdragon.is_alive(self) then
...
function mob:get_wander_pos(min_range, max_range, dir)
	...
	if waterdragon.get_node_def(pos).walkable then -- Occurs if small mob is touching a fence
		...
		pos = waterdragon.get_ground_level(pos, 1)
	end
	...
	if waterdragon.get_node_def(pos2).walkable
	and not dir then
		for _ = 1, 3 do
			move_dir = {
				x = move_dir.z,
			...
			if not waterdragon.get_node_def(pos2).walkable then
				break
			end
		end
	elseif dir then
	...
	for i = 1, outset do
		...
		if waterdragon.get_node_def(a_pos).walkable
		or not waterdragon.get_node_def(b_pos).walkable then
			a_pos = waterdragon.get_ground_level(a_pos, floor(self.stepheight or 1))
		end
		if not waterdragon.get_node_def(a_pos).walkable then
			pos2 = a_pos
		else
			break
		end
...
function mob:get_wander_pos_3d(min_range, max_range, dir, vert_bias)
	...
	for i = 1, outset do
		local a_pos = vec_add(pos2, vec_multi(move_dir, i))
		if waterdragon.get_node_def(a_pos).walkable then
			a_pos = waterdragon.get_ground_level(a_pos, floor(self.stepheight or 1))
		end
		if not waterdragon.get_node_def(a_pos).walkable then
			pos2 = a_pos
...
function mob:is_pos_safe(pos, ignore_liquid)
	if not pos then return end
	local n_def = waterdragon.get_node_def(pos)
	if minetest.get_item_group(n_def.name, "igniter") > 0
	or (not ignore_liquid
	and (n_def.drawtype == "liquid"
	or waterdragon.get_node_def(vec_raise(pos, -1)).drawtype == "liquid")) then return false end
	...
	if self.max_fall ~= 0 then
		for _ = 1, self.max_fall or 3 do
			...
			if waterdragon.get_node_def(fall_pos).walkable then
				fall_safe = true
				break
			end
		end
...
function mob:get_target(target)
	local alive = waterdragon.is_alive(target)
	if not alive then
...
function mob:store_nearby_objects(radius)
	...
	for _, object in ipairs(objects) do
		if waterdragon.is_alive(object)
...
function mob:initiate_utility(utility, ...)
	local func = waterdragon.registered_utilities[utility]
...
local function collision_detection(self)
	if not waterdragon.is_alive(self)
...
function mob:_physics()
	-- Physics
	waterdragon.default_water_physics(self)
...
function mob:move_to(goal, method, speed_factor)
    ...
    -- Get movement method
    local get_method = waterdragon.registered_movement_methods[method]
...
function waterdragon.register_mob(name, def)
	...
	def._waterdragon_mob = true

	def.sounds = def.sounds or {}

	if not def.sounds.hit then
		def.sounds.hit = {
			name = "waterdragon_hit",
			gain = 0.5,
			distance = 16,
			variations = 3
		}
	...
	def._vitals = def._vitals or waterdragon.default_vitals
