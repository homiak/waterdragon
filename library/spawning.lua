...
waterdragon.registered_mob_spawns = {}
waterdragon.registered_on_spawns = {}
	...
	for y = 0, height, height / ceil(height) do
		for z = -width, width, w_iter do
			for x = -width, width, w_iter do
				...
				local def = waterdragon.get_node_def(pos2)
...
local function do_on_spawn(pos, obj)
	...
	local spawn_functions = waterdragon.registered_on_spawns[name] or {}
...
function waterdragon.register_spawn_item(name, def)
	...
	or (def.hex_primary and def.hex_secondary)) then
		...
		local base = "(waterdragon_spawning_crystal_primary.png^[multiply:#" .. primary .. ")"
		local spots = "(waterdragon_spawning_crystal_secondary.png^[multiply:#" .. secondary .. ")"
...
function waterdragon.register_on_spawn(name, func)
	if not waterdragon.registered_on_spawns[name] then
		waterdragon.registered_on_spawns[name] = {}
	end
	table.insert(waterdragon.registered_on_spawns[name], func)
end
...
--[[waterdragon.register_abm_spawn("mymod:mymob", {
	chance = 3000,
	interval = 30,
	min_height = 0,
	max_height = 128,
...
local protected_spawn = minetest.settings:get_bool("waterdragon_protected_spawn", true)
local abr = (tonumber(minetest.get_mapgen_setting("active_block_range")) or 4) * 16
local max_per_block = tonumber(minetest.settings:get("waterdragon_mapblock_limit")) or 12
local max_in_abr = tonumber(minetest.settings:get("waterdragon_abr_limit")) or 24
local min_abm_dist = min(abr / 2, tonumber(minetest.settings:get("waterdragon_min_abm_dist")) or 32)

local mobs_spawn = minetest.settings:get_bool("mobs_spawn") ~= false

local mapgen_mobs = {}

function waterdragon.register_abm_spawn(mob, def)
	...
	local function spawn_func(pos, aocw)
		...
		if group_size > 1 then
			...
			for _ = 1, group_size do
				...
				spawn_pos = waterdragon.get_ground_level({
		...
		minetest.log("action",
			"[waterdragon] [ABM Spawning] Spawned " .. group_size .. " " .. mob .. " at " .. minetest.pos_to_string(pos))
	...
	waterdragon.registered_mob_spawns[mob] = {
...
if mapgen_spawning then
	...
	local c_spawn = minetest.get_content_id("waterdragon:spawn_node")
	...
	minetest.register_on_generated(function(minp, maxp)
		...
		for _, mob_name in ipairs(mapgen_mobs) do
			...
			def = waterdragon.registered_mob_spawns[mob_name]
	...
	local spawn_interval = tonumber(minetest.settings:get("waterdragon_spawn_interval")) or 10

	minetest.register_abm({
		label = "waterdragon Spawning",
		...
		action = function(pos)
			...
			if amount > 0 then
				...
				minetest.log("action",
					"[waterdragon] Spawned " .. amount .. " " .. name .. " at " .. minetest.pos_to_string(pos))
			else
				obj = minetest.add_entity(pos, name)
				do_on_spawn(pos, obj)
				minetest.log("action",
					"[waterdragon] Spawned a " .. name .. " at " .. minetest.pos_to_string(pos))
			end
...
function waterdragon.register_mob_spawn(name, def)
	local spawn_def = {
	...
	--waterdragon.registered_mob_spawns[name] = spawn_def

	waterdragon.register_abm_spawn(name, spawn_def)
end

function waterdragon.register_spawn_egg(name, col1, col2, inventory_image)
	if col1 and col2 then
		local base = "(waterdragon_spawning_crystal_primary.png^[multiply:#" .. col1 .. ")"
		local spots = "(waterdragon_spawning_crystal_secondary.png^[multiply:#" .. col2 .. ")"
		inventory_image = base .. "^" .. spots
	end
	local mod_name = name:split(":")[1]
	local mob_name = name:split(":")[2]
