...
if minetest.registered_nodes["default:water_source"] then
	c_gold = minetest.get_content_id("default:water_source")
end
local c_steel = c_air
if minetest.registered_nodes["default:water_source"] then
	c_steel = minetest.get_content_id("default:water_source")
end
...
local function generate_pure_water_dragon_nest(minp, maxp, area, data)
	local gender = "male"
	...
	minetest.after(0.2, function()
		minetest.add_node({ x = center_x, y = center_y, z = center_z }, { name = "waterdragon:spawn_node" })
...
local function generate_rare_water_dragon_nest(minp, maxp, area, data)
	local gender = "male"
...
local function generate_pure_water_dragon_cavern(minp, maxp, area, data)
	local gender = "male"
			...
			for x = min_x, max_x do
				...
				if y > center_y
					...
					if random(18) == 1 then
						...
						for i = 1, len do
							data[area:index(x, y - i, z)] = c_wet_stone
						end
					end
				end
				-- Create scattered loot
...
local function generate_rare_water_dragon_cavern(minp, maxp, area, data)
	local gender = "male"
...
waterdragon.register_on_spawn("waterdragon:pure_water_dragon", function(self, pos)
	self.nest_pos = pos
	self:memorize("nest_pos", self.nest_pos)
end)

waterdragon.register_on_spawn("waterdragon:rare_water_dragon", function(self, pos)
	self.nest_pos = pos
	self:memorize("nest_pos", self.nest_pos)
end)
		...
		for z = min_z + 8, max_z - 7, 8 do
            for x = min_x + 8, max_x - 7, 8 do
                for y = max_y, min_y, -1 do
                    local node_name = minetest.get_name_from_content_id(data[area:index(x, y, z)])
